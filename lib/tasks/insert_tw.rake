...
CTRL-G             顯示當前的檔名、檔案狀態 (唯讀、已修改 ...) 和游標位置。
<BS>            1  等同於 "h"。
CTRL-H          1  等同於 "h"。
...
CTRL-W ^           分割視窗並編輯備用檔 N。
CTRL-W _           設定當前視窗高度為 N (預設: 儘量大)。
CTRL-W b           跳轉到底部的視窗。
CTRL-W c           關閉當前視窗 (類似於 |:close|)。
...
CTRL-W n           開啟新視窗，N 行高。
CTRL-W o           關閉除了當前視窗以外的所有視窗 (類似於|:only|)。
CTRL-W p           轉到前面一個 (最近訪問的) 視窗。
CTRL-W q           退出當前視窗 (類似於 |:quit|)。
CTRL-W r           向下旋轉視窗 N 次。
CTRL-W s           分割當前視窗為兩部分，新的視窗 N 行高。
CTRL-W t           跳轉到頂部視窗。
CTRL-W v           垂直分割當前視窗，新的視窗 N 列寬。
CTRL-W w           轉到後面第 N 個視窗 (迴繞)。
CTRL-W x           交換當前和第 N 個視窗 (預設: 下一個視窗)。
CTRL-W z           關閉預覽視窗。
CTRL-W |           設定視窗寬度為 N 列。
...
###方括號指令
[ CTRL-D           跳轉到第一個在當前檔案和它包含的標頭檔裡符合游標所在的單字的 #define，從當前檔案的頭部開始。
[ CTRL-I           跳轉到第一個在當前檔案和它包含的標頭檔裡符合游標所在單字的位置，從當前檔案頭部開始。
...
[D                 列出在當前檔案和它包含的標頭檔裡符合游標所在的單字的所有定義，從當前檔案的頭部開始。
[I                 列出在當前檔案和它包含的標頭檔裡符合游標所在單字的所有位置，從當前檔案的頭部開始。
[P              2  等同於 "[p"。
[[              1  游標後退 N 個小節。
[]              1  游標後退 N 個 SECTION。
[c              1  游標後退 N 個更改開始的地方。
[d                 顯示第一個在當前檔案和它包含的標頭檔裡符合游標所在的單字的 #define，從當前檔案的頭部開始。
[f                 等同於 "gf"。
[i                 顯示第一個在當前檔案和它包含的標頭檔裡符合游標所在單字的位置，從當前檔案的頭部開始。
[m              1  游標後退 N 個成員函數的開始。
[p              2  類似於 "P"，但調整該行的縮排。
[s              1  移動到前一個拼字錯誤的單字。
[z              1  移動到開啟的摺疊的開始。
[{              1  游標後退 N 個未符合的 '{'。
[<MiddleMouse>  2  等同於 "[p"。
] CTRL-D           跳轉到第一個在當前檔案和它包含的標頭檔裡符合游標所在的單字的 #define，從游標位置開始。
] CTRL-I           跳轉到第一個在當前檔案和它包含的標頭檔裡符合游標所在單字的位置，從游標位置開始。
...
]D                 列出在當前檔案和它包含的標頭檔裡符合游標所在的單字的所有定義，從游標位置開始。
]I                 列出在當前檔案和它包含的標頭檔裡符合游標所在單字的所有位置，從游標位置開始。
]P              2  等同於 "[p"。
]]              1  游標前進 N 個小節。
]c              1  游標前進 N 個更改開始的地方。
]d                 顯示第一個在當前檔案和它包含的標頭檔裡符合游標所在的單字的 #define，從游標位置開始。
]f                 等同於 "gf"。
]i                 顯示第一個在當前檔案和它包含的標頭檔裡符合游標所在單字的位置，從游標位置開始。
]m              1  游標前進 N 個成員函數的結尾。
]p              2  類似於 "p"，但調整該行的縮排。
]s              1  移動到後一個拼字錯誤的單字。
]z              1  移動到開啟的摺疊的結尾處。
...
gd              1  轉到當前函數裡當前游標所在的單字的定義。
ge              1  反向轉到前一個單字的結尾處。
gf                 開始編輯當前游標所在的名字對應的檔案。
...
:cad[dexpr]     通過表達式補充錯誤。
:caddf[ile]     加入錯誤訊息到當前 quickfix 清單。
:cal[l]         調用函數。
:cat[ch]        :try 指令的一部分。
...
:lvimgrepa[dd]  類似於 :vimgrep，但附加到當前清單。
:lw[indow]      開啟或關閉位置視窗。
:m[ove]         移動行。
:ma[rk]         設定位置標記。
...
:mes[sages]     顯示以前顯示的消息。
:mk[exrc]       寫入當前映射和設定到檔案。
:mks[ession]    寫入 session 資訊到檔案。
:mksp[ell]      生成 .spl 拼寫檔案。
:mkv[imrc]      寫入當前映射和設定到檔案。
:mkvie[w]       寫入當前視窗的視圖到檔案。
:mod[e]         顯示或者變更畫面模式。
...
:vim[grep]      在若干檔案裡尋找模式。
:vimgrepa[dd]   類似於 :vimgrep，但附加到當前清單。
